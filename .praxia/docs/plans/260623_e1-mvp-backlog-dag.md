...
- **D1 — UNKNOWN carrier = an explicit `role` field on `AxisSpec`**, typed `AxisRole` (MVP members: **`KNOWN`** and **`UNKNOWN`**; deferred Tier-2 later adds concrete semantic members — BATCH/SEQUENCE/… — all treated as non-fail-loud), **appended last** (after `bucket_boundaries`) with **`default = AxisRole.KNOWN`**.
  - *Why `KNOWN` and not `UNKNOWN` is the default:* a default-constructed `AxisSpec` is every existing hand-written/positional site — those are user-declared, hence `KNOWN`, so the planner proceeds exactly as today (no fail-loud regression).
  - *The two cases never share the default:* `synthesize_axes` **ALWAYS sets `role` explicitly** — `KNOWN` when the axis is resolved (via `@axis_config` in MVP), **`UNKNOWN` when unresolved** — and never relies on the field default. Only `role == UNKNOWN` triggers the planner fail-loud (E1.3b).
  - Reject sentinel-cardinality (corrupts `eda/stats.py:56,118,130`, which read `spec.cardinality`) and reject a parallel UNKNOWN set (changes `plan()` signature).
...
|----|------|------|-----|-----------|--------------------|
| **E1.0**-pkg-errors | S | — | enables AC3,AC5 | no (smoke import) | new `xtrax.inference` pkg; `AmbiguousAxisError`, `StructureMismatchError`, `AxisRole` enum (`KNOWN` = default/proceed, `UNKNOWN` = fail-loud sentinel; Tier-2 extends). No jaxtyping import. |
...
| **E1.3a**-axisspec-role-field | M | E1.2 | (enables AC3) | yes | **SPLIT from E1.3 (fix #4).** Append `role` field LAST on `AxisSpec` (`default=AxisRole.KNOWN`, per D1) + `synthesize_axes`/`resolve_axis_role` (Tier-1 `@axis_config` hook → else explicit `UNKNOWN`). **NO planner behavior change.** Regression tests: existing positional sites construct unchanged AND **`AxisSpec('batch',64,128).role == KNOWN` (NOT UNKNOWN)**; a `synthesize_axes` unresolved-axis output has **`.role == UNKNOWN`**; `AxisSpec` still hashable (set test); `batch_size`/`granularity` deprecation shim still warns (fix #2). |
| **E1.3b**-batchplanner-failloud | M | E1.3a | AC3, AC2(axis-cov) | yes | **KEYSTONE enforcement.** Insert UNKNOWN check in `plan()` per-spec loop, **after Phase-0/0b, before `_decide_strategy`** → `AmbiguousAxisError(axis, how-to-resolve)`. Drop the bogus `:159` cite (fix #1). Red test: UNKNOWN→raises `AmbiguousAxisError`; the **residues-first protein fn from the spec pre-mortem** (named fixture) is NEVER auto-batched; existing `tiling/test_plan.py` stays green. |
...
## Status
Revised DAG addresses all 8 required fixes + the AC8 suggestion.

**Re-audit (Opus, 260623):** 7/8 fixes RESOLVED & source-verified; verdict NEEDS_WORK on a single mechanical blocker — the D1 default-value contradiction (default both "concrete" and `AxisRole.UNKNOWN`'s counterpart; `synthesize_axes`-sets-UNKNOWN-explicitly not stated). **Now fixed above:** `default = AxisRole.KNOWN`; `synthesize_axes` always sets `role` explicitly (`KNOWN` resolved / `UNKNOWN` unresolved); E1.3a regression test pins both halves. The auditor confirmed the E1.3a/3b split is safe (grep: zero existing `resolve_axis_role`/`UNKNOWN`/`synthesize_axes` call sites → 3a is purely additive), the real fail-loud insertion point is `plan.py:222-225` (between Phase-0/0b `continue` and `_decide_strategy`), and the E1.4→E1.3a re-point drops nothing.

Recommended next: a D1-scoped confirmatory re-audit, then load into the praxia backlog (`depends_on` per the table) and route E1.3a/E1.3b through `spec_driven_dev` with the keystone red tests written first.
