...
status: FINAL r5 — adversarially converged, challenger round 3 verdict ACCEPT (7 MINOR conditions, binding in §14)
revision: 5 (260922; r4 folded in the round-3 conditions as §14; r5 corrects AC-35 and AC-22 after the audit and the first CI run)
---
...
| r5 | 260922 | Post-implementation corrections from the audit and the first CI run on PR #158:<br>- **AC-35** is restated as a PR-time check. The unit test that encoded it shelled out to `git diff origin/main`, which failed in CI because the checkout has no `origin/main` ref.<br>- **AC-22** gains a host-path int4 case. A mutation audit found the numpy-side sub-byte widening untested: every int4 case was a `jax.Array`. numpy int4 has a non-canonical high nibble, so without widening, `verify_dedup_spec` falsely rejects a correct spec.<br>- The **AC-45 checker** also walks argument default values. |
...
| AC-35 | regression, **PR-time check, not a unit test** (r5) | run `tests/tiling/test_dedup_synthesis.py` (CI does this on every build); at PR time, confirm the file is absent from `git diff origin/main...HEAD --stat` | all tests pass, including `TestJaxArrayNoMaterialization`; the file is not in the PR diff | — (r5: a unit test that shells out to `git diff origin/main` failed in CI, where the checkout has no `origin/main` ref, and after merge it would fail forever and forbid any future edit of the file. It was removed.) |
