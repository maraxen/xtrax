#!/usr/bin/env python3
"""D8 structure-complexity gate CLI — complexipy + ruff C901/PLR091x (N2.8 / #1588)."""
...
from xtrax.devtools.gates.structure_complexity import run_structure_complexity_gate
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Baseline JSON for structure complexity metrics",
    )
    parser.add_argument(
        "--cognitive-threshold",
        type=int,
        default=15,
        help="complexipy threshold for per-function hit detection",
    )
    parser.add_argument(
        "--cognitive-ceiling",
        type=int,
        default=25,
        help="Emit findings for cognitive complexity above this ceiling",
    )
    parser.add_argument(
        "--no-write-baseline",
        action="store_true",
    ...
    result = run_structure_complexity_gate(
        ...
        cognitive_ceiling=args.cognitive_ceiling,
    ...
    print(
        f"{status}: {result.cognitive_metric_key}="
        f"{result.cognitive_complexity_max:.0f} "
        f"{result.ruff_metric_key}={result.ruff_violation_count} "
