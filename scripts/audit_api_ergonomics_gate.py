#!/usr/bin/env python3
"""D6 API-ergonomics gate CLI — param sprawl + baseline ratchet (N2.7 / #1586)."""
...
from xtrax.devtools.gates.api_ergonomics import run_api_ergonomics_gate
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Baseline JSON for api_ergonomics.param_sprawl_violation_count",
    )
    parser.add_argument(
        "--max-required",
        type=int,
        default=5,
        help="Maximum allowed required parameters before flagging sprawl",
    )
    parser.add_argument(
        "--no-write-baseline",
        action="store_true",
    ...
    result = run_api_ergonomics_gate(
        ...
        max_required=args.max_required,
    ...
    print(
        f"{status}: api_ergonomics.param_sprawl_violation_count="
